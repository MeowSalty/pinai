...
// Adapter 适配器，将内部 health.Storage 转换为 portal 需要的 health.Storage 接口。
type Adapter struct {
	storage HealthStorage
}

// New 创建健康存储适配器。
func New(healthStorage HealthStorage) *Adapter {
	return &Adapter{storage: healthStorage}
}

// Get 实现 portal health.Storage 接口的 Get 方法。
func (a *Adapter) Get(resourceType coreHealth.ResourceType, resourceID uint) (*coreHealth.Health, error) {
	...
	return &coreHealth.Health{
		...
		LastErrorMessage:        internalResult.LastErrorMessage,
		LastStructuredErrorCode: internalResult.LastStructuredErrorCode,
		LastHTTPStatus:          internalResult.LastHTTPStatus,
		LastErrorFrom:           internalResult.LastErrorFrom,
		LastCauseMessage:        internalResult.LastCauseMessage,
...
func (a *Adapter) Set(status *coreHealth.Health) error {
	...
	internalStatus := &types.Health{
		...
		LastStructuredErrorCode: status.LastStructuredErrorCode,
		LastHTTPStatus:          status.LastHTTPStatus,
		LastErrorFrom:           status.LastErrorFrom,
		LastCauseMessage:        status.LastCauseMessage,
...
func (a *Adapter) Delete(resourceType coreHealth.ResourceType, resourceID uint) error {
	// 将 portal 库的 ResourceType 转换为内部 health 包的 ResourceType
	internalResourceType := convertResourceTypeToInternal(resourceType)
