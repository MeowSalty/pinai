...
// Repository 是 portal 适配层专用的数据访问实现。
//
...
func (r *Repository) CreateRequestLog(ctx context.Context, log *request.RequestLog) error {
	...
	dbLog := &types.RequestLog{
		...
		CauseMessage:         log.CauseMessage,
		ErrorCode:            log.ErrorCode,
		ErrorLevel:           log.ErrorLevel,
		HTTPStatus:           log.HTTPStatus,
		ErrorFrom:            log.ErrorFrom,
		UpstreamErrorType:    log.UpstreamErrorType,
		UpstreamErrorCode:    log.UpstreamErrorCode,
		UpstreamErrorParam:   log.UpstreamErrorParam,
		UpstreamErrorMessage: log.UpstreamErrorMessage,
		UpstreamRequestID:    log.UpstreamRequestID,
		ResponseBodyIsJSON:   log.ResponseBodyIsJSON,
		ResponseBodyRaw:      log.ResponseBodyRaw,
