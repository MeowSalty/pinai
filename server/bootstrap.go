...
// bootstrapRuntime 持有应用启动后运行所需的基础依赖。
...
// newBootstrapRuntime 完成应用启动装配，并返回可运行的运行时对象。
func newBootstrapRuntime(cfg *config.Config) *bootstrapRuntime {
	// 初始化日志记录器
...
// start 启动 HTTP 服务监听。
func (r *bootstrapRuntime) start() {
	go func() {
		if err := r.srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			r.ginLogger.Error("无法启动 Web 服务", "error", err)
			os.Exit(1)
		}
	}()
}

// waitForShutdownSignal 阻塞等待进程退出信号。
func (r *bootstrapRuntime) waitForShutdownSignal() {
	c := make(chan os.Signal, 1)
	signal.Notify(c, os.Interrupt, syscall.SIGINT, syscall.SIGTERM)
	<-c
}

// shutdown 按既有顺序关闭应用依赖。
func (r *bootstrapRuntime) shutdown() {
	...
	if err := r.srv.Shutdown(ctx); err != nil {
		r.ginLogger.Error("关闭 Web 服务失败", "error", err)
	} else {
		r.ginLogger.Info("Web 服务已成功关闭")
	}

	// 关闭数据库连接
	if err := r.db.Close(); err != nil {
		r.appLogger.Error("关闭数据库连接失败", "error", err)
	} else {
		r.appLogger.Info("数据库连接已成功关闭")
	}

	r.closeLogFile()
	r.appLogger.Info("应用已成功关闭")
}
